...
- You may not change the code you are reviewing, and that includes side
  effects: running the application, a formatter, or a build can drop files
  into the repo, and one changed byte anywhere in `{{work_root}}` is rolled
  back and fails the whole phase. Prefer read-only invocations. Write your
  review to `<context_handoff_dir>`, which is outside the repo.
...
- For exact byte inspection, use the read-only one-input form `xxd <file>` plus `wc`; do not use inline Python or shell substitutions. Use ordinary `git status`, `git diff`, `git log`, and `git show` forms without `%`-based custom formats.
