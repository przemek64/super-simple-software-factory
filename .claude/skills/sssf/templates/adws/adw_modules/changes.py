...
def resolve_base(ref: str, *, cwd: Path) -> BaseRef:
...
def capture(run, params: ChangeCapture) -> ChangeSet:
    ...
    cwd = run.work_root
    base = resolve_base(params.base, cwd=cwd)
    files = git_helper.diff_files(base.commit, cwd=cwd)
    untracked = git_helper.untracked_files(cwd=cwd) if params.include_untracked else []
    insertions, deletions = git_helper.diff_counts(base.commit, cwd=cwd)
    stat = git_helper.diff_stat(base.commit, cwd=cwd)

    text = git_helper.diff_text(base.commit, cwd=cwd)
    ...
    diff_path.write_text(
        f"# changes since {base.label} @ {git_helper.short_sha(base.commit, cwd=cwd)}\n"
        f"# {base.reason}\n"
...
def as_envelope(changes: ChangeSet, notes: str = "", *, cwd: Path) -> ChangesOutput:
    ...
    return ChangesOutput(
        ...
        base=f"{changes.base.label} @ {git_helper.short_sha(changes.base.commit, cwd=cwd)} "
